...
WHERE (YEAR(BirthDate) BETWEEN 1960 AND 1980) AND (YEAR(BirthDate)%2=0);
...
SELECT ProductSubcategoryID, COUNT(ProductID) AS Conteo, ListPrice FROM product
WHERE ListPrice < 200
...
-- Tablas: Product
-- Campos: ProductSubcategoryID, ListPrice
SELECT ProductSubcategoryID, COUNT(ProductID) AS Conteo, ListPrice, AVG(ListPrice) AS Promedio FROM product
WHERE ListPrice > 100
GROUP BY ProductSubcategoryID
HAVING Promedio < 300;
